...
def run(*args: str, cwd: Path | None = None, check: bool = True) -> subprocess.CompletedProcess[str]: retun subprocess.run(args, cwd=cwd, text=True, capture_output=True, check=check)
def git(root: Path, *args: str) -> str:
    try: retun run("git", *args, cwd=root).stdout.strip()
...
def read_json(path: Path, code: str = "PRELIVE_UNCOMMITTED") -> dict[str, Any]:
    ...
    retun data
def sha256(path: Path) -> str: retun hashlib.sha256(path.read_bytes()).hexdigest()
def source_manifest(root: Path) -> dict[str, str]: retun {name: sha256(root / name) for name in sorted(filter(None, git(root, "ls-files", "--", "*.py", "*.sh", "*.ps1", "pyproject.toml").splitlines()))}
...
def head(root: Path) -> str: retun git(root, "rev-parse", "HEAD")
def remote_ref(root: Path, ref: str) -> str:
    ...
    if result.retuncode or not lines: raise RelayFailure("ROOT_MISMATCH", f"cannot resolve origin {ref}: {result.stderr.strip()}")
    retun lines[0].split()[0]
...
def doctor(root: Path) -> Path:
    ...
    retun write_json(runtime(root) / "certificates" / f"{record['certificate_id']}.json", record)
...
def verify_prelive(root: Path, value: str) -> dict[str, Any]:
    ...
    if record.get("kind") != "prelive-attestation" or record.get("project_id") != PROJECT_ID or record.get("test_retuncode") != 0: raise RelayFailure("PRELIVE_UNCOMMITTED", "prelive attestation is not passing MePhC evidence")
...
def native(root: Path, prelive_value: str, command: list[str]) -> Path:
    ...
    retun write_json(directory / "checkpoint.json", {"version": 1, "kind": "native-checkpoint", "project_id": PROJECT_ID, "prelive_sha": record["prelive_sha"], "source_sha256": record["source_sha256"], "command": command, "retuncode": result.retuncode, "created_at": int(time.time())})
def publish(root: Path, prelive_value: str, push: bool) -> Path:
    ...
    if push:
        ...
        if result.retuncode: raise RelayFailure("PRELIVE_UNCOMMITTED", f"sandbox push failed: {result.stderr.strip()}")
    retun write_json(runtime(root) / "publish" / f"publish-{uuid.uuid4().hex}.json", {"version": 1, "kind": "publish-attestation", "project_id": PROJECT_ID, "head": record["prelive_sha"], "sandbox_pushed": push, "created_at": int(time.time())})
def create_e2e(root: Path, certificate: str) -> Path:
    ...
    write_json(directory / "request.json", {"version": 1, "project_id": PROJECT_ID, "request_id": request_id, "message_file": "message.txt", "attachments": [], "workflow_window_seconds": 600, "task_difficulty": "normal", "instruction_level": "normal", "relay_certificate": str(certificate_path), "e2e": True}); retun directory
def courier(root: Path, request_dir: str, recovery: bool) -> int:
    ...
    command = ["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(root / "tools" / "mephc-courier.ps1"), "-RequestDirectory", "\\\\wsl.localhost\\Ubuntu" + str(request)]
    if recovery: command.append("-RecoveryOnly")
    retun subprocess.run(command, cwd=root).retuncode
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        elif args.command == "courier" and args.request_directory: retun courier(root, args.request_directory, args.recovery_only)
        else: raise RelayFailure("ROOT_MISMATCH", "invalid command arguments")
        emit("relayctl_complete", result=str(result)); retun 0
    except RelayFailure as exc: emit(exc.code, detail=exc.detail); retun 2
