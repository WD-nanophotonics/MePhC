...
def courier(root: Path, request_dir: str, recovery: bool) -> int:
    ...
    command = ["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", str(root / "tools" / "mephc-courier.ps1"), "-RequestDirectory", "\\\\wsl.localhost\\Ubuntu" + str(request).replace("/", "\\\\")]
