...
def test_static_contracts_encode_exact_counts_and_no_source_mutation():
    ...
    assert machine["future_live_request_count"] == machine["future_provider_budget"] == machine["future_solver_budget"] == 72
    ...
    assert "import meep" not in source.lower()
    assert "import mpb" not in source.lower()
