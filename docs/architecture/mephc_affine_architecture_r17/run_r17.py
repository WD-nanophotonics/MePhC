...
def analyze(fresh, controls):
    ...
    pair_sign=[math.copysign(1,out["pair"]["112"]["per_phase_alpha"][key(p)]["alpha"])==math.copysign(1,alphas["pair"]["112"]) for p in PHASES]; full_sign=[math.copysign(1,out["full"]["112"]["per_phase_alpha"][key(p)]["alpha"])==math.copysign(1,alphas["full"]["112"]) for p in PHASES]
...
def emit(a, ledger, controls):
    ...
    write("uncertainty_budget.json", {"pair":{"components":a["components"]["pair"],"u":a["uncertainty"]["pair"]},"full":{"components":a["components"]["full"],"u":a["uncertainty"]["full"]},"uniform_matched_floor":a["uniform_matched_floor"],"raw_uniform_included_if_transferability_fails":a["raw_uniform_stress_nontransferable"]})
