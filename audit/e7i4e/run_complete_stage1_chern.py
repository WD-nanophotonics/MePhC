...
def local_profile(records,source_d,cache,identities,counters,adapter,preflight,geometry,element_id):
    ...
    for idx,rec in enumerate(records):
        ...
        q=tuple(rec["actual_public_q"]); source_row=source_d.get(element_id) if idx==len(records)-1 else None
...
def evaluate(element,delta,ref_delta,adapter,preflight,geometry,cache,identities,counters,source_d):
    ...
    recs=[v[2] for v in vals]+[cv[2]]; prof,low=local_profile(recs,source_d,cache,identities,counters,adapter,preflight,geometry,element["element_id"])
    area=abs(float(np.linalg.det(np.asarray(preflight.delta_k_vectors_to_public_q(delta))))); qualified=path.status==PATH_SUBSPACE_QUALIFIED and wil.status==WILSON_LOOP_QUALIFIED and boundary.is_qualified and interior.is_qualified and (refine is None or (refine.is_qualified and nxt["profile_passed"])) and prof
    out={"local_delta_k":delta,"vertices_q":[list(q) for q in qs],"center_q":list(center),"path_status":path.status,"edges":[{"status":e.status,"external_gaps":dict(e.external_gaps),"evidence":list(e.evidence),"min_sv":None if e.overlap is None else e.overlap.min_singular_value,"angle":None if e.overlap is None else e.overlap.max_principal_angle,"projector_distance":e.projector_distance} for e in path.edge_results],"boundary_status":boundary.status,"interior_spoke_status":interior.status,"refinement":refine_summary,"wilson_status":wil.status,"determinant_phase":wil.determinant_phase,"low_gap_profile":low,"low_gap_profile_flat":low+([] if refine is None else nxt["low_gap_profile_flat"]),"profile_passed":prof and (True if refine is None else nxt["profile_passed"]),"qualified":qualified,"omega_trace_q":None if not qualified else float(-wil.determinant_phase/area),"solve_records":recs}
...
def main():
    ...
    def source_d_map():
        return {x["element_id"]:x["ladder"][1] for x in d["blockers"]}
