...
def main():
    ...
    blob_b=subprocess.check_output(["git","show",f"{SOURCE_E7I4B_COMMIT}:{SOURCE_E7I4B_PATH}"],cwd=root)
    if sha(blob_b)!=SOURCE_E7I4B_SHA: raise RuntimeError("E7I.4B source SHA mismatch")
    source_b=json.loads(blob_b.decode()); source_b_map={x["element_id"]:x for x in source_b["elements"]}
    ...
    for item in blockers:
        baseline={"resolution":48,"evaluation_q":item["evaluation_q"],"frequencies_bands_1_to_4":source_b_map[item["element_id"]]["original"]["selected_span_solves"][-1]["frequencies_bands_1_to_4"],"G34":item["center_band4_minus_band3_gap"],"G34_over_f3":None,"G34_over_f4":None,"absolute_005_policy_status":policy(item["center_band4_minus_band3_gap"]),"finite_external_gap_status":finite_status(item["center_band4_minus_band3_gap"])}
        ladder=[baseline]+[solve_center(adapter,res,item["evaluation_q"],counters) for res in RESOLUTIONS]
        repeats=[solve_center(adapter,R96,item["evaluation_q"],counters)]
        gaps=[x["G34"] for x in ladder]; pair={"element_id":item["element_id"],"evaluation_q":item["evaluation_q"],"element_weight":item["element_area_weight"],"nearest_outer_edge":item["nearest_outer_edge"],"gamma_exclusion_distances":item["distance_to_each_Gamma_exclusion_boundary"],"R48_center_gap":item["center_band4_minus3_gap"] if "center_band4_minus3_gap" in item else item["center_band4_minus_band3_gap"],"ladder":ladder,"R96_repeats":repeats,"R96_repeat_gap_abs_difference":abs(ladder[2]["G34"]-repeats[0]["G34"]),"R96_repeat_frequency_max_abs_difference":max(abs(a-b) for a,b in zip(ladder[2]["frequencies_bands_1_to_4"],repeats[0]["frequencies_bands_1_to_4"])),"delta_gap_48_64":gaps[1]-gaps[0],"delta_gap_64_96":gaps[2]-gaps[1],"delta_gap_96_128":gaps[3]-gaps[2],"final_pair_abs_drift":abs(gaps[3]-gaps[2]),"gap_trend":classify(gaps),"R128_gap":gaps[3],"R128_absolute_005_policy":policy(gaps[3]),"R128_finite_external_gap":finite_status(gaps[3])}
        rows.append(pair)
    ...
    result={"schema":"e7i4d_two_blocker_gap_convergence_v1","work_order":WORK_ORDER,"code_change":"SANDBOX_AUDIT_ONLY","source_e7i4c_evidence_commit":SOURCE_COMMIT,"source_e7i4c_result_path":SOURCE_PATH,"source_e7i4c_result_sha256":source_sha,"blocker_reconstruction":"EXACT_2","blocker_count":2,"blockers":rows,"blocker_pair_symmetry_relation":"VERIFIED" if pair_sym else "NOT_VERIFIED","pair_gap_differences":pair_diffs,"new_R64_solves":2,"new_R96_solves":2,"new_R128_solves":2,"R96_repeat_solves":2,"total_new_mpb_solves":counters["new_solves"],"solver_failures":counters["solver_failures"],"total_wall_time_seconds":time.monotonic()-started,"scale_aware_isolation_diagnostic":"NOT_EVALUATED","current_absolute_005_policy":"UNCHANGED_AUTHORITATIVE","physical_rank3_degeneracy_confirmed":False,"local_berry_calculation":"NOT_AUTHORIZED","stage1_chern":"NOT_AUTHORIZED","stage2_chern":"NOT_AUTHORIZED","per_band_chern":"NOT_AUTHORIZED","rank2_chern":"NOT_AUTHORIZED","full_bz_chern":"NOT_AUTHORIZED","bcd":"NOT_AUTHORIZED","deformation_physics":"NOT_AUTHORIZED","blocker_1_gap_trend":rows[0]["gap_trend"],"blocker_2_gap_trend":rows[1]["gap_trend"],"blocker_1_R128_gap":rows[0]["R128_gap"],"blocker_2_R128_gap":rows[1]["R128_gap"],"blocker_1_R128_absolute_005_policy":rows[0]["R128_absolute_005_policy"],"blocker_2_R128_absolute_005_policy":rows[1]["R128_absolute_005_policy"],"blocker_1_finite_external_gap":rows[0]["R128_finite_external_gap"],"blocker_2_finite_external_gap":rows[1]["R128_finite_external_gap"],"R96_repeatability":"PASSED" if all(x["R96_repeat_gap_abs_difference"]==0.0 and x["R96_repeat_frequency_max_abs_difference"]==0.0 for x in rows) else "FAILED","R48_blocker_interpretation":"RESOLUTION_UNRESOLVED","current_005_policy_potentially_compatible_with_paper_domain":"UNRESOLVED","physical_rank3_degeneracy_confirmed":False,"historical_absolute_005_policy":"UNCHANGED_AUTHORITATIVE","local_berry_calculation":"NOT_AUTHORIZED","stage1_chern":"NOT_AUTHORIZED","blocker_1_gap_trend":rows[0]["gap_trend"],"blocker_2_gap_trend":rows[1]["gap_trend"],"blocker_1_R128_gap":rows[0]["R128_gap"],"blocker_2_R128_gap":rows[1]["R128_gap"],"blocker_1_R128_absolute_005_policy":rows[0]["R128_absolute_005_policy"],"blocker_2_R128_absolute_005_policy":rows[1]["R128_absolute_005_policy"],"blocker_1_finite_external_gap":rows[0]["R128_finite_external_gap"],"blocker_2_finite_external_gap":rows[1]["R128_finite_external_gap"],"R96_repeatability":"PASSED" if all(x["R96_repeat_gap_abs_difference"]==0.0 and x["R96_repeat_frequency_max_abs_difference"]==0.0 for x in rows) else "FAILED","R48_blocker_interpretation":"RESOLUTION_UNRESOLVED","current_005_policy_potentially_compatible_with_paper_domain":"UNRESOLVED","physical_rank3_degeneracy_confirmed":False,"historical_absolute_005_policy":"UNCHANGED_AUTHORITATIVE","local_berry_calculation":"NOT_AUTHORIZED","stage1_chern":"NOT_AUTHORIZED","calculation_logic_git_sha":subprocess.check_output(["git","rev-parse","HEAD"],cwd=root,text=True).strip(),"calculation_logic_committed_before_execution":True,"main_unchanged":True,"sandbox_remote_head_verified":False,"e7i4d_overall":"TWO_BLOCKER_RESOLUTION_DIAGNOSIS_READY_FOR_SUPERVISOR_DECISION"}
