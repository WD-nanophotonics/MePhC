"""M64R4R1: recover exact Build5 binding, semantic probes, and loaded-lib proof."""
...
RESULT_SCHEMA = "mephc-berry-c3-consistency-m64r4r1-recover-isolated-localize-patch-ab-v1"
DATASET_SCHEMA = "mephc-berry-c3-consistency-m64r4r1-patched-frequency-ab-dataset-v1"
...
def _safe(value: Any) -> Any:
    ...
    raise ValueError(f"M64R4R1_UNSAFE_RESULT:{type(value).__name__}")


def _load(path: Path, name: str) -> Any:
    spec = importlib.util.spec_from_file_location(name, path)
    require(spec is not None and spec.loader is not None, "M64R4R1_IMPORT_FAILED", str(path))
    module = importlib.util.module_from_spec(spec); spec.loader.exec_module(module); return module


def _read_dataset(job: Any, root: Path, binding: tuple[str, str, str, int]) -> list[dict[str, Any]]:
    dataset_id, manifest, schema, count = binding; verified = job.verify_dataset(root, dataset_id)
    require(verified.get("dataset_id") == dataset_id and verified.get("manifest_sha256") == manifest, "M64R4R1_DATASET_BINDING_INVALID", dataset_id)
    require(verified.get("record_count") == count, "M64R4R1_DATASET_COUNT_INVALID", dataset_id); rows = []
    for key in verified["record_key_sha256"]:
        row = json.loads(job.resolve_dataset_record(root, dataset_id, manifest, key)["payload"].decode("utf-8")); require(row.get("schema") == schema, "M64R4R1_DATASET_SCHEMA_INVALID", dataset_id); rows.append(row)
    return rows
...
def _index_prior(rows: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    indexed = {str(row.get("probe")): row for row in rows}
    require(len(indexed) == 4 and all(key and key != "None" for key in indexed), "M64R4R1_PRIOR_PROBE_KEYS_INVALID")
    return indexed
...
def loaded_library_ledger(maps_text: str, prefix: str, expected_sha: str) -> dict[str, Any]:
    ...
    for line in maps_text.splitlines():
        path_text = line.split(maxsplit=5)[-1] if len(line.split(maxsplit=5)) >= 6 else ""
        if "libmpb.so" not in path_text:
            continue
        path = Path(path_text).resolve()
        if path.is_file() and str(path).startswith(str(root) + os.sep):
            candidates.append(path)
    unique = sorted(set(candidates)); require(len(unique) == 1, "M64R4R1_LOADED_LIB_AMBIGUOUS")
    actual = _hash(unique[0]); require(actual == expected_sha, "M64R4R1_LOADED_LIB_SHA_MISMATCH")
    return {"loaded_path": str(unique[0]), "loaded_sha256": actual, "under_isolated_prefix": True, "path_source": "proc_self_maps"}
...
def main() -> int:
    ...
    try:
        job = _load(ROOT / "tools/mephc-flow/scientific_job.py", "m64r4r1_job"); state_root = Path(os.environ["MEPHC_EXECUTION_COUNTERS_PATH"]).parent.parent
        prior = _read_dataset(job, state_root, M64R3); prior_by_probe = _index_prior(prior); _read_dataset(job, state_root, M50); _read_dataset(job, state_root, M60); m61_rows = _read_dataset(job, state_root, M61); m63_rows = _read_dataset(job, state_root, M63)
        m61 = {(int(row["vertex_index"]), int(row["repeat_index"]), str(row["c3_member_identity"])): row for row in m61_rows}; failures = _failure_set(m61); require(failures, "M64R4R1_M63_FAILURE_NOT_REPRODUCED"); worst = sorted(failures, key=lambda item: (-(item["residual"] - item["combined_repeat_uncertainty"]), -item["residual"], item["vertex"], item["band"], item["source_member"], item["target_member"]))[0]
        expected_keys = {f"unpatched:frozen:v{worst['vertex']}:{member}" for member in MEMBERS} | {"unpatched:generic:k0"}; require(set(prior_by_probe) == expected_keys, "M64R4R1_PRIOR_SEMANTIC_KEYS_INVALID")
        coordinates = {member: list(m61[(worst["vertex"], 0, member)]["coordinate"]) for member in MEMBERS}; require(len({tuple(value) for value in coordinates.values()}) == 3, "M64R4R1_MEMBER_COORDINATES_NOT_DISTINCT")
        for member in MEMBERS: require(prior_by_probe[f"unpatched:frozen:v{worst['vertex']}:{member}"].get("coordinate") == coordinates[member], "M64R4R1_PRIOR_COORDINATE_MISMATCH", member)
        require(all(row.get("schema") == "mephc-berry-c3-consistency-m63-homogeneous-raw-mode-tolerance-dataset-v1" for row in m63_rows), "M64R4R1_M63_SCHEMA_INVALID")
        previous = _load(ROOT / "audit/berry_c3_consistency/m64r3_dynamic_source_discovery_localize_patch_ab.py", "m64r3_discovery"); discovery = previous.discover_source(); identity = _verify_binding(); counter = job.BudgetCounter(0, 80)
        source = ROOT / "vendor/mpb_c3_patch/source/mpb-1.12.0.tar.gz"; driver = ROOT / "vendor/mpb_c3_patch/build_exact_build5.sh"
        with tempfile.TemporaryDirectory(prefix="m64r4r1-build-") as temporary:
            root = Path(temporary); build_dir = root / "build"; prefix = root / "prefix"; completed = subprocess.run(["bash", str(driver), str(source), str(build_dir), str(prefix)], env=os.environ.copy(), capture_output=True, text=True, timeout=900, check=False); require(completed.returncode == 0, "M64R4R1_ISOLATED_BUILD_FAILED", (completed.stdout + "\n" + completed.stderr)[-4000:]); libraries = sorted((prefix / "lib").glob("libmpb.so*")); require(libraries, "M64R4R1_ISOLATED_LIBRARY_MISSING")
            probes = [(f"unpatched:frozen:v{worst['vertex']}:{member}", coordinates[member], True) for member in MEMBERS] + [("unpatched:generic:k0", [0.123456, -0.210987], False)]
            for label, coordinate, frozen in probes:
                row = _child_probe(prefix, coordinate, label, counter, frozen); installed = prior_by_probe[label]["observed_frequencies_first4"]; row["installed_baseline_frequencies_first4"] = installed; row["installed_baseline_max_difference"] = float(max(abs(float(a) - float(b)) for a, b in zip(row["observed_frequencies_first4"], installed))); row["baseline_equivalence"] = row["installed_baseline_max_difference"] <= 1e-9; records.append({"schema": DATASET_SCHEMA, "work_order_id": bundle["work_order_id"], "arm": "UNPATCHED_ISOLATED_TRACE", "source_commit": source_commit, **row})
        require(all(row["baseline_equivalence"] for row in records), "M64R4R1_ISOLATED_BASELINE_NOT_REPRODUCED")
        namespace = {"goal_id": "MEPHC-BERRY-C3-CONSISTENCY-V1", "work_order_id": bundle["work_order_id"], "source_commit": source_commit, "record_schema": DATASET_SCHEMA}; store = job.ImmutableDatasetStore(state_root, namespace); require(not store.root.exists(), "M64R4R1_DATASET_NAMESPACE_EXISTS")
