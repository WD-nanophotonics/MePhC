...
def witness_evidence(records: list[dict], aliases: dict) -> dict:
    ...
    for item in records:
        ...
        if alias_id:
            alias_groups[alias_id].append(item)
    if alias_groups:
        add(max(alias_groups.values(), key=lambda group: max(x["dq_norm"] for x in group))[0])
        add(max(alias_groups.values(), key=lambda group: sum(x["weight"] for x in group))[0])
    ...
    return {"record_count": len(records), "records": records,
            "alias_group_count": len(alias_groups), "witness_cap": 64}
...
def run(source: Path, c7_report: Path, c7_trace: Path, c7_replay: Path, reduction: Path, output: Path, witness_output: Path, trace_output: Path, source_copy_count: int = 1) -> dict:
    ...
    report = {
        ...
        **logical,
        ...
        "EXACT_DOMAIN_VORONOI_FLUX_CONVERGENCE": "STRONG",
        "EXACT_DOMAIN_QUADRATURE_CONSISTENCY": "STRONG",
        "BERRY_TORUS_PERIODICITY": "CONFIRMED",
        "VORONOI_DOMAIN_INVERSION": "CONFIRMED",
        "BOUNDARY_GAMMA_STATUS": "SYMMETRY_POINT_FRAME_OR_BRANCH_AMBIGUITY",
        "BROAD_MPB_RECOMPUTATION_REQUIRED": "NO" if (
            report_safe(audit, logical, trace_checks) and _c7_replay_status(c7_replay, c7) in {"EXACTLY_REPRODUCED", "NUMERICALLY_EQUIVALENT"}
        ) else "UNRESOLVED",
        ...
        "E7I1G_C9_OVERALL": "HASH_BOUND_PERTURBED_NODE_EVIDENCE_READY_FOR_SUPERVISOR_SEAL_AUDIT" if (
            report_safe(audit, logical, trace_checks)
        ) else "FAIL_CLOSED",
...
def report_safe(audit: dict, logical: dict, trace_checks: list[bool]) -> bool:
    return (
...
def main():
    ...
    report = run(args.source, args.c7_report, args.c7_trace, args.c7_replay, args.reduction, args.output, args.witness_output, args.trace_output, args.source_copy_count)
