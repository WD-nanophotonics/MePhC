...
def normalize(record):
    ...
    return result
def controls(path):
    value=json.loads(path.read_text()); sentinels=[item["r64"] for item in value["resolution_sentinels"]]; s1=statistics.median(abs(float(item["omega_band1"])) for item in sentinels); s2=statistics.median(abs(float(item["omega_band2"])) for item in sentinels)
