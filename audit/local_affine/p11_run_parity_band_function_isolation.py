"""Repeat the P10 probes with correct Git blob identity verification."""
...
WORK_ORDER_ID = "MEPHC-LOCALAFFINE-P11-RUN-PARITY-BAND-FUNCTION-ISOLATION-20260829-375"
BASE_INPUT_COMMIT = "48c899cafed79a331428fc65e205537a23a039f4"
...
HISTORICAL_RUN_BLOB = "5119df243617ebedaac615a07305c77dacf4de83"
HISTORICAL_ENERGY_BLOB = "f3dd87c9b3ece5ccec577caa4b1b47b269e53133"
HISTORICAL_GEOMETRY_BLOB = "53beca884f7f2802cf152e5c32d468ad35efda3d"
...
def write_marker(path: Path, probe: str, stage: str, event: str) -> None:
    line = f"P11_STAGE|{probe}|{stage}|{event}\n".encode("ascii")
...
def worker(probe: str, marker_path: Path, fault_path: Path) -> int:
    with fault_path.open("ab", buffering=0) as fault_file:
        ...
        require(identity["polarization"] == "TM" and identity["resolution"] == 64
                ...
                "P11_STATE_01_SOLVER_CONTRACT_INVALID")
        ...
        values = provider.solve(tuple(identity["public_q"]))
        require(values.shape == (6,), "P11_RESULT_SHAPE_INVALID")
...
def probe_result(probe: str, completed: subprocess.CompletedProcess[bytes], marker_path: Path) -> dict[str, Any]:
    ...
    frequency_ok = any("|FREQUENCY_EXTRACTION|EXIT" in line for line in markers)
    success = completed.returncode == 0 and any("|RUN_PARITY|EXIT" in line for line in markers) and frequency_ok
    return {
        "probe_id": probe, "return_code": completed.returncode, "sigsegv": completed.returncode == -11,
        "last_stage_marker": markers[-1] if markers else "NONE", "frequency_extraction_success": frequency_ok,
        "finite_six_frequencies": frequency_ok, "success": success, "top_faulthandler_frame": fault_frame(Path(str(marker_path) + ".faulthandler.log")),
        "markers": markers, "stdout": (completed.stdout or b"").decode("utf-8", errors="replace"),
...
def classify(a: dict[str, Any], b: dict[str, Any]) -> tuple[str, str, str]:
    if a["success"] and b["success"]:
        return ("ZERO_BAND_FUNCTION_RUN_PARITY_NATIVE_BUG", "ESTABLISHED",
                "PASS_A_SEMANTICS_NEUTRAL_NOOP_BAND_FUNCTION_WHEN_NO_GAUGE_CALLBACK_IS_REQUESTED_THEN_CERTIFY_ONE_COMPLETE_H_SNAPSHOT_BEFORE_FULL_ACQUISITION")
    ...
    if a["success"] and not b["success"]:
        return ("FIX_EFIELD_PHASE_CALLBACK_SPECIFIC_FAILURE", "LOCALIZED_NOT_ESTABLISHED",
                "NOOP_CALLBACK_CANDIDATE_PENDING_COMPLETE_H_SNAPSHOT_CERTIFICATION")
    return ("HISTORICAL_E8B_RUN_PATH_NO_LONGER_REPRODUCES", "LOCALIZED_NOT_ESTABLISHED", "NONE")
...
def main() -> int:
    if len(sys.argv) == 4 and sys.argv[1] == "--p11-worker":
    ...
    require(counters_path.name, "P11_COUNTER_PATH_MISSING")
    require(sha256_file(P9_BINDING) == P9_BINDING_SHA, "P9_BINDING_HASH_MISMATCH")
    p9 = json.loads(P9_BINDING.read_text(encoding="utf-8"))
    require(p9.get("RUN_PARITY_FAILURE_CLASS") == "GENERAL_E8B_MPB_RUN_PATH"
            and p9.get("PROBE_A_RESULT", {}).get("sigsegv") is True
            and p9.get("PROBE_B_RESULT", {}).get("sigsegv") is True,
            "P9_BINDING_EVIDENCE_INVALID")
    require(git_blob("audit/e8b/run_e8b.py") == HISTORICAL_RUN_BLOB,
            "HISTORICAL_RUN_GIT_BLOB_ID_MISMATCH")
    require(git_blob("mephc/mpb_energy_spectral_provider.py") == HISTORICAL_ENERGY_BLOB,
            "HISTORICAL_ENERGY_PROVIDER_GIT_BLOB_ID_MISMATCH")
    require(git_blob("audit/e8b/e8b_geometry.py") == HISTORICAL_GEOMETRY_BLOB,
            "HISTORICAL_GEOMETRY_GIT_BLOB_ID_MISMATCH")
    require(sha256_file(PROVIDER_PATH) == PROVIDER_SHA and sha256_file(GRAPH_PATH) == GRAPH_SHA,
            "P11_CURRENT_INPUT_HASH_MISMATCH")
    historical_run = (ROOT / "audit/e8b/run_e8b.py").read_text(encoding="utf-8")
    historical_energy = (ROOT / "mephc/mpb_energy_spectral_provider.py").read_text(encoding="utf-8")
    require("phase_callback=mpb.fix_efield_phase" in historical_run and "R64=64" in historical_run
            and "for res in (R48,R64)" in historical_run and "polarization=mp.TM" in historical_run,
    ...
    runtime = load_module("_mephc_p11_runtime", ROOT / "tools/mephc-flow/mephc_science_runtime.py")
    scientific_job = load_module("_mephc_p11_scientific_job", ROOT / "tools/mephc-flow/scientific_job.py")
    require(scientific_job.runtime_hash(ROOT) == RUNTIME_SHA, "P11_RUNTIME_HASH_MISMATCH")
    state_root = runtime._trusted_science_state_root()
    trace_namespace = {
        "project_id": "MEPHC", "science_contract_id": WORK_ORDER_ID, "source_commit": execution_source,
        "entrypoint_sha256": sha256_file(ROOT / "audit/local_affine/p11_run_parity_band_function_isolation.py"),
        "trace_type": "p11_run_parity_band_function_isolation", "p9_trace_dataset_id": P9_TRACE_DATASET_ID,
    }
    trace_store = scientific_job.ImmutableDatasetStore(state_root, trace_namespace)
    require(not trace_store.root.exists(), "P11_TRACE_NAMESPACE_ALREADY_EXISTS")
    ...
    for probe in ("TM_FALSE_NOOP_BAND_FUNCTION", "TM_FALSE_FIX_EFIELD_PHASE"):
        ...
        completed = subprocess.run(
            [sys.executable, str(Path(__file__).resolve()), "--p11-worker", probe, str(marker_path)],
    ...
    payload = canonical({
        "schema": "mephc-local-affine-p11-run-parity-band-function-diagnostic-trace-v1",
        "work_order_id": WORK_ORDER_ID, "science_execution_source": execution_source,
        "historical_git_blob_identity_status": "PASS", "p9_trace_dataset_id": P9_TRACE_DATASET_ID,
        "p9_trace_manifest_sha256": P9_TRACE_MANIFEST_SHA, "probe_a": a, "probe_b": b,
    ...
    result = {
        "schema": "mephc-local-affine-p11-run-parity-band-function-isolation-v1",
        ...
        "MACHINE_CONTRACT_STATUS": "PASS", "HISTORICAL_GIT_BLOB_IDENTITY_STATUS": "PASS",
        "HISTORICAL_RUN_GIT_BLOB_ID": HISTORICAL_RUN_BLOB, "HISTORICAL_ENERGY_PROVIDER_GIT_BLOB_ID": HISTORICAL_ENERGY_BLOB,
        "HISTORICAL_GEOMETRY_GIT_BLOB_ID": HISTORICAL_GEOMETRY_BLOB, "HISTORICAL_E8B_CONTRAST_STATUS": "PASS",
        "P9_BINDING_SHA256": P9_BINDING_SHA, "P9_TRACE_DATASET_ID": P9_TRACE_DATASET_ID,
        "P9_TRACE_MANIFEST_SHA256": P9_TRACE_MANIFEST_SHA, "PROVIDER_MODULE_SHA256": PROVIDER_SHA,
        ...
        "PROBE_A_RETURN_CODE": a["return_code"], "PROBE_A_SIGSEGV": a["sigsegv"], "PROBE_A_LAST_STAGE_MARKER": a["last_stage_marker"],
        "PROBE_A_FREQUENCY_EXTRACTION_SUCCESS": a["frequency_extraction_success"], "PROBE_A_FINITE_SIX_FREQUENCIES": a["finite_six_frequencies"],
        "PROBE_A_SUCCESS": a["success"], "PROBE_A_TOP_FAULTHANDLER_FRAME": a["top_faulthandler_frame"],
        "PROBE_B_RETURN_CODE": b["return_code"], "PROBE_B_SIGSEGV": b["sigsegv"], "PROBE_B_LAST_STAGE_MARKER": b["last_stage_marker"],
        "PROBE_B_FREQUENCY_EXTRACTION_SUCCESS": b["frequency_extraction_success"], "PROBE_B_FINITE_SIX_FREQUENCIES": b["finite_six_frequencies"],
        "PROBE_B_SUCCESS": b["success"], "PROBE_B_TOP_FAULTHANDLER_FRAME": b["top_faulthandler_frame"],
        "BAND_FUNCTION_FAILURE_CLASS": failure_class, "ROOT_CAUSE_STATUS": root_cause,
        ...
        "LOCALAFFINE_P11_BAND_FUNCTION_ISOLATION_STATUS": "PASS", "P9_DIAGNOSIS_BINDING_STATUS": "VERIFIED",
        ...
        "TERMINAL": "LOCALAFFINE_P11_RUN_PARITY_BAND_FUNCTION_ISOLATION_COMPLETE",
    }
