"""Flatten the verified P17 outcome from the exact P18 result file."""
...
WORK_ORDER_ID = "MEPHC-LOCALAFFINE-P20-P17-OUTCOME-FLATTEN-20260830-384"
...
def main() -> int:
    ...
    require(isinstance(nested, dict), "P17_NESTED_RESULT_MISSING")
    require(nested.get("work_order_id") == P17_WORK_ORDER_ID, "P17_NESTED_WORK_ORDER_INVALID")
    require(nested.get("classification") == "P17_PRODUCTION_PROVIDER_FAILURE_CAPTURED",
            "P17_CHILD_OUTCOME_INVALID")
    require(nested.get("parent_result_written_even_if_child_fails") is True,
            "P17_PARENT_RESULT_FLAG_INVALID")

    flattened = {
        "classification": nested.get("classification"),
        "child_return_code": nested.get("child_return_code"),
        "stage_trace": nested.get("stage_trace", []),
        "faulthandler_enabled": nested.get("faulthandler_enabled"),
        "durable_stage_trace_captured": nested.get("durable_stage_trace_captured"),
        "parent_result_written_even_if_child_fails": nested.get("parent_result_written_even_if_child_fails"),
        "exact_production_local_affine_provider_path_used": nested.get("exact_production_local_affine_provider_path_used"),
    }
    write_result({
        "schema": "mephc-local-affine-p20-p17-outcome-flatten-v1",
        "work_order_id": WORK_ORDER_ID,
        "prior_result_job_id": P18_JOB_ID,
        "prior_result_source_commit": P18_SOURCE_COMMIT,
        "p18_result_schema_verified": True,
        "p18_result_source_verified": True,
        "nested_p17_result_verified": True,
        "p17_child_outcome_flattened": True,
