"""Repeat the complete P44 STATE_01 certification once, without root-cause inference."""
...
WORK_ORDER_ID = "MEPHC-LOCALAFFINE-P46-STATE01-P44-EXACT-RECERTIFICATION-20260830-410"
...
def load_budget_counter() -> Any:
    ...
    spec = importlib.util.spec_from_file_location("_mephc_p46_scientific_job", path)
...
def verify_production_blobs() -> dict[str, str]:
    observed: dict[str, str] = {}
    for path in PRODUCTION_PATHS:
        original = git_blob(ORIGINAL_P44_SOURCE_COMMIT, path)
        current = git_blob("HEAD", path)
        require(current == original, f"PRODUCTION_BLOB_CHANGED:{path}")
        observed[path] = current
    return observed


def validate_snapshot(snapshot: Any, spec: Any, identity: dict[str, Any]) -> None:
    ...
    require(np.allclose(np.asarray(reciprocal[:2], dtype=float), np.asarray(identity["derived_kappa"]),
...
def main() -> int:
    ...
    blobs = verify_production_blobs()
    BudgetCounter = load_budget_counter()
    counter = BudgetCounter(1, 1)
    with tempfile.TemporaryDirectory(prefix="mephc-p46-") as temporary:
    ...
    write_result({
        "schema": "mephc-local-affine-p46-state01-p44-exact-recertification-v1",
        "work_order_id": WORK_ORDER_ID,
        "parent_work_order_id": "MEPHC-LOCALAFFINE-P45-P44-CHILD-FAILURE-LOCALIZATION-20260830-409",
        "source_commit": os.environ.get("MEPHC_SOURCE_COMMIT", ""),
        "original_p44_source_commit": ORIGINAL_P44_SOURCE_COMMIT,
        "production_blob_equivalence_to_p44": True,
        "verified_production_blob_count": len(blobs),
        "production_code_changed": False,
        "p44_prior_failure_classification": "UNREPRODUCED",
        "p44_prior_failure_root_cause_inferred": False,
        "static_matrix_passed_before_live_execution": True,
        **worker_result,
