...
def _identity(item: dict[str, Any], snapshot: Any, binding: dict[str, Any]) -> PhaseSpaceStateIdentity:
    ...
    if not isinstance(state_identity, dict):
        state_identity = {}
    public_q = _finite_vector(supplied.get("public_q", binding["public_q"]), 2, "P66_PUBLIC_Q_INVALID")
    s = float(supplied.get("s", binding["s"]))
    derived = _finite_vector(supplied.get("derived_kappa", state_identity.get("derived_kappa", public_q)), 2, "P66_DERIVED_KAPPA_INVALID")
    matrix = lambda key, default: tuple(tuple(float(x) for x in row) for row in np.asarray(supplied.get(key, state_identity.get(key, default)), dtype=float))
    A_s = matrix("A_s", ((1.0, 0.0), (0.0, 1.0)))
    F_s = matrix("F_s", ((1.0, 0.0), (0.0, 1.0)))
    reference = provenance.get("local_affine_reference_cell_contract", {})
    if not isinstance(reference, dict):
        reference = {}
    ref = ReferenceCellIdentity(
        resolution=int(reference.get("resolution", 64)),
        spatial_shape=tuple(int(x) for x in getattr(snapshot, "spatial_shape", (64, 64))),
        lattice_size=tuple(reference.get("lattice_size", (1.0, 1.0))),
        component_order=str(reference.get("component_order", "supplied final axis order")),
        component_basis=str(reference.get("component_basis", "LAB_CARTESIAN")),
        mu_contract=str(reference.get("mu_contract", "MU1_NONMAGNETIC")),
        orientation_sign=int(reference.get("orientation_sign", 1)),
        fractional_material_indexing_identity=str(reference.get("fractional_material_indexing_identity", "same fractional (ix,iy) material coordinates")),
        reference_cell_identity=str(reference.get("reference_cell_identity", "certified-common-reference-cell")),
    )
    return PhaseSpaceStateIdentity(
        public_q=public_q, s=s, derived_kappa=derived, A_s=A_s, F_s=F_s,
        geometry_identity=str(supplied.get("geometry_identity", state_identity.get("geometry_digest", "local-affine-bound"))),
        reference_cell=ref,
        solver_configuration_identity=str(supplied.get("solver_configuration_identity", "resolution64-bands6-tm-tol1e-7-mesh3-deterministic")),
    )
