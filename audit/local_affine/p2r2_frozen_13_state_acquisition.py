"""Execute the frozen P2R1 state graph with exact historical reconciliation."""
...
WORK_ORDER_ID = "MEPHC-LOCALAFFINE-P2R2-FROZEN-13-STATE-LIVE-ACQUISITION-20260829-363"
BASE_INPUT_COMMIT = "31646d54daba115e1379acf87f0c970c8e44fbec"
...
FAILED_P2_ORDER = "MEPHC-LOCALAFFINE-P2-FROZEN-13-STATE-LIVE-ACQUISITION-20260829-361"
FAILED_P2R1_ORDER = "MEPHC-LOCALAFFINE-P2R1-FROZEN-13-STATE-LIVE-ACQUISITION-20260829-362"
FAILED_P2_ENTRYPOINT_SHA = "3c8ea4af35ea1f6a921b6b52a33215fc4289204a331efe853f33cfb4ac865a02"
FAILED_P2R1_ENTRYPOINT_SHA = "df09ebd383880b306f447a3be6b3270fdbb9ae12f38e014e82cb9d98c59dffb0"
...
def reconcile_failed_attempt(science_jobs: Path, native_runs: Path, work_order_id: str,
                             expected_source: str, expected_error: str,
                             allow_underlying_path_error: bool = False) -> dict[str, Any]:
    ...
    require(tuple(native.get(field, 0) for field in fields) == (0, 0, 0),
            "FAILED_NATIVE_SIDE_EFFECT_COUNTS_NONZERO")
    stderr_path = native_runs / f"{native_run_id}.stderr.log"
    stderr = stderr_path.read_text(encoding="utf-8")
    if allow_underlying_path_error:
        require(expected_error in stderr or "FileNotFoundError" in stderr, "FAILED_P2R1_EXACT_ERROR_UNCONFIRMED")
    else:
        require(expected_error in stderr, "FAILED_P2_EXACT_ERROR_UNCONFIRMED")
    return {"job_id": job["job_id"], "source_sha": source, "return_error": expected_error,
            "provider_count": 0, "solver_count": 0, "dataset_count": 0}


def verify_inputs(counters_path: Path) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    failed_p2 = reconcile_failed_attempt(science_jobs, native_runs, FAILED_P2_ORDER, "872efed7f7fb79bc6335d083343c2bb5144ffde3", "SCIENCE_SOURCE_COMMIT_INVALID")
    failed_p2r1 = reconcile_failed_attempt(science_jobs, native_runs, FAILED_P2R1_ORDER, BASE_INPUT_COMMIT,
    ...
    require(sha256_file(OLD_P2R1_ENTRYPOINT) == FAILED_P2R1_ENTRYPOINT_SHA, "FAILED_P2R1_ENTRYPOINT_MUTATED")
...
def verify_snapshot(snapshot: Any, spec: Any, identity: dict[str, Any], expected_shape: tuple[int, int]) -> dict[str, Any]:
    ...
    require(snapshot.component_count == 3 and tuple(snapshot.spatial_shape) == expected_shape, "PROVIDER_RESULT_SHAPE_INVALID")
    ...
    require(provenance.get("local_affine_state_identity") == identity, "LOCAL_AFFINE_STATE_IDENTITY_MISMATCH_BEFORE_PERSISTENCE")
    require(provenance.get("local_affine_state_identity_sha256") == digest_state_identity(identity), "LOCAL_AFFINE_STATE_IDENTITY_DIGEST_MISMATCH_BEFORE_PERSISTENCE")
    lattice = spec.geometry_lattice.size
    expected_contract = local_affine_reference_cell_contract(spec, spatial_shape=expected_shape, identity=identity,
                                                              lattice_size=(float(lattice.x), float(lattice.y)))
    require(provenance.get("local_affine_reference_cell_contract") == expected_contract, "REFERENCE_CELL_CONTRACT_MISMATCH_BEFORE_PERSISTENCE")
    ...
    if isinstance(caller, dict) and "mpb_reciprocal_k_point" in caller:
        require(np.allclose(np.asarray(caller["mpb_reciprocal_k_point"], dtype=float), reciprocal, rtol=0.0, atol=1e-9), "CALLER_RECIPROCAL_METADATA_MISMATCH")
    frequencies = np.asarray(snapshot.frequencies, dtype=float)
    raw_norms = np.asarray(snapshot.raw_norms, dtype=float)
    require(frequencies.shape == (6,) and np.all(np.isfinite(frequencies)) and np.all(frequencies > 0.0), "PERIODIC_H_FREQUENCIES_INVALID")
    require(raw_norms.shape == (6,) and np.all(np.isfinite(raw_norms)) and np.all(raw_norms > 0.0), "PERIODIC_H_NORMS_INVALID")
    ...
    for vector in snapshot.normalized_vectors:
        ...
        require(np.isfinite(norm) and np.isclose(norm, 1.0, rtol=0.0, atol=1e-10), "PERIODIC_H_NORMALIZED_VECTOR_INVALID")
        norms.append(norm)
...
def main() -> int:
    ...
    failed_p2, failed_p2r1 = verify_inputs(counters_path)
    graph = load_graph()
    current_states = state_set(graph)
    require(state_set_digest(current_states) == STATE_SET_SHA and len(current_states) == 13, "SCIENTIFIC_STATE_SET_IDENTITY_MISMATCH")
    ...
    runtime = load_module("_mephc_p2r2_science_runtime", ROOT / "tools/mephc-flow/mephc_science_runtime.py")
    scientific_job = load_module("_mephc_p2r2_scientific_job", ROOT / "tools/mephc-flow/scientific_job.py")
    ...
    require(certification.get("schema") == "mephc-science-runtime-certification-v1" and smoke.get("executed") is True, "SCIENCE_RUNTIME_MPB_SMOKE_NOT_CERTIFIED")
    namespace = {"project_id": "MEPHC", "science_contract_id": WORK_ORDER_ID, "source_commit": execution_source,
                 "entrypoint_sha256": sha256_file(ROOT / "audit/local_affine/p2r2_frozen_13_state_acquisition.py"),
                 "request_graph_sha256": sha256_file(GRAPH_PATH)}
    store = scientific_job.ImmutableDatasetStore(state_root, namespace)
    require(not store.root.exists(), "P2R2_DATASET_NAMESPACE_ALREADY_EXISTS")
    import meep as mp
    ...
    for item, spec in zip(graph["states"], specs):
        ...
        identity_metadata = {**identity, "state_id": item["state_id"], "role": item["role"], "key_identity": key_identity,
                             ...
                             "request_graph_identity": namespace["request_graph_sha256"], "scientific_state_set_identity": STATE_SET_SHA,
    ...
    result = {"WORK_ORDER_ID": WORK_ORDER_ID, "BASE_INPUT_COMMIT": BASE_INPUT_COMMIT,
              ...
              "MACHINE_CONTRACT_STATUS": "PASS", "FAILED_P2_SCIENCE_JOB_ID": failed_p2["job_id"],
              "FAILED_P2_SCIENCE_SOURCE_SHA": failed_p2["source_sha"], "FAILED_P2_RETURN_ERROR": failed_p2["return_error"],
              "FAILED_P2_PROVIDER_EXECUTION_COUNT": 0, "FAILED_P2_DATASET_SOLVER_EXECUTION_COUNT": 0, "FAILED_P2_DATASET_RECORD_COUNT": 0,
              "FAILED_P2R1_SCIENCE_JOB_ID": failed_p2r1["job_id"], "FAILED_P2R1_SCIENCE_SOURCE_SHA": failed_p2r1["source_sha"],
              "FAILED_P2R1_RETURN_ERROR": failed_p2r1["return_error"], "FAILED_P2R1_PROVIDER_EXECUTION_COUNT": 0,
              "FAILED_P2R1_DATASET_SOLVER_EXECUTION_COUNT": 0, "FAILED_P2R1_DATASET_RECORD_COUNT": 0,
              "FAILED_P2_EXECUTION_RECONCILED": True, "FAILED_P2R1_EXECUTION_RECONCILED": True,
              "DURABLE_FLOW_ROOT_BINDING_STATUS": "PASS", "EXECUTION_SOURCE_BINDING_STATUS": "PASS",
              "REQUEST_GRAPH_SHA256": namespace["request_graph_sha256"], "REQUEST_GRAPH_IDENTITY_STATUS": "PASS",
              "SCIENTIFIC_STATE_SET_IDENTITY": STATE_SET_SHA, "SCIENTIFIC_STATE_SET_IDENTITY_STATUS": "PASS",
              "P1R1_PROVIDER_CERTIFICATION_STATUS": "VERIFIED", "SCIENCE_RUNTIME_MPB_SMOKE_STATUS": "PASS",
              "SCIENCE_RUNTIME_MPB_SMOKE_SOLVER_EXECUTIONS": int(smoke.get("solver_executions", 0)), "PROVIDER_MODULE_SHA256": PROVIDER_SHA,
              "SCIENCE_JOB_ID": job_id, "SCIENCE_SOURCE_SHA": execution_source, "LOCAL_AFFINE_P2R2_ACQUISITION_STATUS": "PASS",
              "FROZEN_STATE_GRAPH_STATUS": "PASS", "E8B_GEOMETRY_BINDING_STATUS": "PASS", "FIXED_Q_TO_LOCAL_KAPPA_STATUS": "PASS",
              "CANONICAL_RECIPROCAL_METADATA_STATUS": "PASS", "REFERENCE_CELL_METADATA_STATUS": "PASS", "PERIODIC_H_SNAPSHOT_STATUS": "PASS",
              "RANK1_PREFLIGHT_ISOLATION_STATUS": "PASS", "MIN_BAND0_EXTERNAL_GAP": minimum_gap, "DATASET_ID": manifest["dataset_id"],
              "DATASET_MANIFEST_SHA256": manifest["manifest_sha256"], "DATASET_RECORD_COUNT": 13, "COMPLETED_STATE_COUNT": 13,
              "FAILED_STATE_COUNT": 0, "NATIVE_INVOCATION_COUNT": 1, "PROVIDER_EXECUTION_COUNT": 13, "SOLVER_EXECUTION_COUNT": 13,
              "CACHE_REUSE_COUNT": 0, "RETRY_COUNT": 0, "ORIGINAL_FAILED_SOLVE_REUSED": False, "FAILED_P2_PAYLOAD_REUSED": False,
              "FAILED_P2R1_PAYLOAD_REUSED": False, "MPB_EXECUTION": True, "LOCAL_AFFINE_P2R2_DATASET_STATUS": "COMPLETE_IMMUTABLE_13_STATE",
              "LOCAL_AFFINE_P2R2_DATASET_READY_FOR_REDUCTION": True, "LOCAL_AFFINE_LIVE_ACQUISITION_EXECUTED": True,
              "NEXT_LIVE_SOLVER_AUTHORIZATION": False, "PIPELINE_HEALTH": "HEALTHY", "BLOCKED_BY_INFRASTRUCTURE": False,
              "SCIENTIFIC_WORK_MUST_STOP": False,
              "NEXT_SCIENTIFIC_STATE": "LOCAL_AFFINE_P2R2_DATASET_READY_FOR_SOLVER_FREE_TWO_SCALE_BERRY_MIXED_CURVATURE_AND_LOCAL_BAND_DERIVATIVE_REDUCTION",
              "RETURN_TO_SUPERVISOR": True, "TERMINAL": "LOCALAFFINE_P2R2_FROZEN_13_STATE_LIVE_ACQUISITION_COMPLETE"}
