"""RP3.B fixed-six R160 diagnostic runtime; audit-only orchestration."""
...
WORK_ORDER = "MEPHC-E9F-C1-RP3-B-20260825-262"
PHASE = "E9F.C1.RP3.B"
BASE_SANDBOX_SHA = "a3925e136d37fafa4689119ab0963d9b2711306d"
SOURCE_EXECUTION_SHA = "39b63b80bf42d48e5b256dd1a211f6bec2585cd4"
RESOLUTION = 160
STENCILS = ("1/72", "1/144")
CANARY_SOURCE_SAMPLE_ID = "fr=0;grid_i=-5;grid_j=0;estimator=SOURCE_GRID"
CHECKPOINT_SCHEMA = "mephc_e9f_c1_rp3_b_r160_checkpoint_v1"
...
def build_plan(root: Path) -> list[dict[str, Any]]:
    ...
    for index, sample_id in enumerate(policy["rp2_diagnostic_matrix"]["fixed_sample_ids"]):
        ...
        rows.append({"sample_id": f"{sample_id}::resolution=160", "source_sample_id": sample_id, "source_sample_index": int(source["sample_index"]), "sample_index": index, "resolution": RESOLUTION, "authoritative_coordinate": [float(x) for x in source["center"]]})
    if len(rows) != 6 or [row["sample_index"] for row in rows] != list(range(6)) or len({row["source_sample_id"] for row in rows}) != 6:
        raise ValueError("RP3_B_FIXED_SIX_PLAN_FAIL_CLOSED")
    return rows
...
def validate_payload(payload: Mapping[str, Any], *, row: Mapping[str, Any], expected: Mapping[str, Any]) -> None:
    ...
    if payload.get("work_order_id") != WORK_ORDER or payload.get("phase") != PHASE or payload.get("resolution") != RESOLUTION or payload.get("provider", {}).get("resolution") != RESOLUTION:
        raise ValueError("RP3_B_PAYLOAD_IDENTITY_FAIL_CLOSED")
    if payload.get("replay_matched_point_count") != 0 or payload.get("replay_unmatched_point_count") != 9:
        raise ValueError("RP3_B_REPLAY_NOT_APPLICABLE_BINDING_FAIL_CLOSED")
    for point in payload.get("all_point_metrics", []):
        replay = point.get("frequency_replay", {})
        if replay.get("matched") is not False or replay.get("reason") != "R160_NOT_APPLICABLE_ORIGINAL_RP2_HAS_NO_R160_KEY":
            raise ValueError("RP3_B_REPLAY_POLICY_FAIL_CLOSED")
    if payload.get("payload_body_sha256") != body_hash(payload):
        raise ValueError("RP3_B_BODY_HASH_FAIL_CLOSED")
...
def execution_order(rows: Sequence[Mapping[str, Any]]) -> list[str]:
    ...
    if len(canary) != 1 or len(rest) != 5:
        raise ValueError("RP3_B_EXECUTION_ORDER_FAIL_CLOSED")
    return canary + rest
...
def scan_rp3_orphans(worker_ids: Sequence[str]) -> list[int]:
    ...
    for worker_id in worker_ids:
        found.extend(c4_process.scan_orphans("rp3_b_r160_worker.py", worker_id))
...
def _validate_checkpoint_item(item: Mapping[str, Any], row: Mapping[str, Any], checkpoint: Mapping[str, Any], *, root: Path, expected_generation: int) -> None:
    ...
    if any(key not in item for key in required):
        raise ValueError("RP3_B_CHECKPOINT_ITEM_FIELDS_FAIL_CLOSED")
    ...
    if any(item.get(key) != value for key, value in expected.items()) or item.get("item_generation") != expected_generation or item.get("terminal_payload_status") != "COMPLETE":
        raise ValueError("RP3_B_CHECKPOINT_ITEM_IDENTITY_FAIL_CLOSED")
    path = Path(item["payload_path"])
    if not path.is_file() or sha(path) != item["payload_file_sha256"]:
        raise ValueError("RP3_B_CHECKPOINT_FILE_HASH_FAIL_CLOSED")
    payload = json.loads(path.read_text())
    if payload.get("payload_body_sha256") != item["payload_body_sha256"]:
        raise ValueError("RP3_B_CHECKPOINT_BODY_HASH_FAIL_CLOSED")
    ...
    for payload in payloads:
        ...
        for stencil in STENCILS:
            for branch, key in (("band2", "BAND2"), ("band3", "BAND3")):
                ...
                r160 = payload["stencils"][stencil][("BAND2_PHYSICAL_BRANCH_SHADOW" if branch == "band2" else "BAND3_PHYSICAL_BRANCH_SHADOW")]["OMEGA_RANK1_SHADOW"]
                d_old = values[1] - values[0]; d_new = r160 - values[1]
                rows.append({"source_sample_id": sample, "branch": branch, "stencil": stencil, "omega_R64": values[0], "omega_R96": values[1], "omega_R160": r160, "delta_R64_to_R96_signed": d_old, "delta_R64_to_R96_abs": abs(d_old), "delta_R96_to_R160_signed": d_new, "delta_R96_to_R160_abs": abs(d_new), "contraction_ratio_diagnostic_only": None if d_old == 0 else abs(d_new) / abs(d_old)})
        l0 = payload["center"]["L0"]
        spectral.append({"source_sample_id": sample, "center_L0_R160": l0, "R160_REPLAY_POLICY": "NOT_APPLICABLE_R160"})
...
def canonical_rp3_b_c1_process_registry(*, closed: bool = False) -> dict[str, Any]:
    ...
    return {"schema": "mephc_e9f_c1_rp3_b_c1_process_registry_v1", "work_order_id": "MEPHC-E9F-C1-RP3-A-C1-20260825-258", "pipeline_health": "PIPELINE_HEALTHY_WITH_TECH_DEBT" if closed else "PIPELINE_REQUIRES_CORRECTIVE", "incidents": incidents, "p0_items": [], "p1_items": list(sorted(open_ids)), "p2_items": []}


def validate_rp3_b_c1_process_registry(registry: Mapping[str, Any], *, closed: bool = False) -> None:
    expected = canonical_rp3_b_c1_process_registry(closed=closed)
    if dict(registry) != expected:
        raise ValueError("RP3_B_C1_PROCESS_REGISTRY_CANONICAL_STATE_FAIL_CLOSED")
