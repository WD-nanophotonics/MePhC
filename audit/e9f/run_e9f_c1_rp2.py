...
def run_reaped_child(command, worker_id: str, *, timeout_seconds: float = 720.0): return _impl.run_reaped_child(command, worker_id, timeout_seconds=timeout_seconds)
...
def _fixed_rank1_level(*args, **kwargs):
    ...
    if result.get("status") == "DIAGNOSTIC_REPORTED":
        stencil = kwargs.get("stencil") or args[-1]
        h = 1.0 / float(str(stencil).split("/")[1])
        phase = result.get("wilson_phase_wrapped")
        result["PLAQUETTE_AREA_Q"] = h * h
        result["OMEGA_RANK1_SHADOW"] = None if phase is None else float(phase / (h * h))
    return result
