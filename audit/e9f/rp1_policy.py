"""Canonical semantic lock and diagnostic validation for the immutable RP1 contract."""
...
POLICY_FILE_SHA256 = "75f2d32853ab7e0a5878c19a732f4ac91ef993c105a8000b87e4a8a6ed6d5145"
POLICY_CANONICAL_SEMANTIC_SHA256 = "cfbe71ff9f648048901038823c25ffd358bb8a80394fe05d082a57957acfc84a"
...
REQUIRED_L2_OUTPUTS = {"external_gap_below_pair", "external_gap_above_pair", "minimum_external_pair_gap", "minimum_singular_value", "principal_angle_diagnostic", "projector_distance_diagnostic", "rank2_wilson_determinant_phase"}
REQUIRED_DISTINCTIONS = {"MATHEMATICAL_EXACT_DEGENERACY", "POSITIVE_SMALL_GAP", "NUMERICALLY_UNSTABLE_RANK1_STATE"}
...
def canonical_semantic_sha256(contract: Mapping[str, Any]) -> str:
...
def _source_records(source: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for diagnostic in source.get("diagnostics", []):
        if diagnostic.get("classification") not in {FINITE, LOW_GAP}:
            continue
        records.append({"sample_id": diagnostic.get("sample_id"), "sample_index": diagnostic.get("sample_index"), "center": diagnostic.get("center"), "classification": diagnostic.get("classification"), "classification_subtype": diagnostic.get("classification_subtype"), "center_gap": diagnostic.get("existing_numerical", {}).get("center_gap")})
    return records
...
def validate_policy_contract(contract: Mapping[str, Any], root: Path) -> None:
    ...
    if not contract_path.is_file() or _sha256(contract_path) != POLICY_FILE_SHA256:
        raise RP1ContractError("POLICY_CONTRACT_FILE_SHA256_MISMATCH")
    ...
    if matrix.get("optional_escalation_present") is not False or matrix.get("diagnostic_only") is not True or matrix.get("terminal") != "SIX_POINT_DIAGNOSTIC_EVIDENCE_COLLECTED":
        raise RP1ContractError("RP2_SEMANTIC_TERMINAL_MUTATED")
    ...
    if contract["low_gap_policy"].get("must_not_lower_threshold_to_qualify") is not True or set(contract["low_gap_policy"].get("required_distinction", [])) != REQUIRED_DISTINCTIONS:
        raise RP1ContractError("LOW_GAP_SEMANTIC_GATE_MUTATED")
    ...
    if set(l0_fields) != required_l0 or contract["l0_exact_output_contract"].get("qualification_decision_emitted") is not False:
        raise RP1ContractError("L0_OUTPUT_CONTRACT_MUTATED")
    l1 = contract.get("l1_exact_shadow_contract", {})
    if l1.get("bands_zero_based") != [2, 3] or l1.get("principal_branch") != "(-pi, pi]" or l1.get("diagnostic_only") is not True or l1.get("reducer_admissible") is not False or l1.get("association_quality_required") is not True:
        raise RP1ContractError("L1_SHADOW_CONTRACT_MUTATED")
    ...
    if l2.get("pair_zero_based") != [2, 3] or l2.get("thresholds") != {"MIN_EXTERNAL_PAIR_GAP": 0.02, "MIN_SIGMA": 0.9, "MAX_PRINCIPAL_ANGLE": 0.45, "MAX_PROJECTOR_DISTANCE": 0.3} or set(l2.get("required_outputs", [])) != REQUIRED_L2_OUTPUTS:
        raise RP1ContractError("L2_CONTRACT_MUTATED")
    l3 = contract.get("l3_consistency_metric", {})
    if l3.get("metric") != "DELTA_PHASE_RANK1SUM_RANK2DET=abs(Arg(exp(i*(PHI_BAND2+PHI_BAND3-PHI_RANK2_DET))))" or l3.get("record_metric_only") is not True or "consistency_passed" in l3:
    ...
    if anchor.get("source_anchor_available_to_diagnostic_runner") is not False or anchor.get("no_source_paper_target_values") is not True or anchor.get("no_anchor_comparison_in_rp2_output") is not True:
    ...
    _require_false(contract.get("execution_authorization", {}), ["band2_recovery_execution_authorized", "berry_calculation_authorized", "chern_calculation_authorized", "live_mpb_authorized", "main_push_authorized", "scientific_sample_solves_authorized", "three_band_aggregate_authorized", "threshold_change_authorized"])
    if canonical_semantic_sha256(contract) != POLICY_CANONICAL_SEMANTIC_SHA256:
        raise RP1ContractError("POLICY_CANONICAL_SEMANTIC_DIGEST_MISMATCH")
...
def validate_manifest_path_set(manifest: Mapping[str, Any], actual_paths: Iterable[str]) -> None:
    ...
    if "audit/e9f/rp1_c2_provenance_manifest.json" not in declared:
