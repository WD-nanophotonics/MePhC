"""RP3.B fixed-six R160 diagnostic runner; no reducer or scientific verdict."""
...
WORKER = Path("audit/e9f/rp3_b_r160_worker.py")
CONTRACT = Path("audit/e9f/rp3_b_r160_execution_contract.json")
...
def verify_provenance(root: Path) -> dict[str, Any]:
    expected = {
        root / "audit/e9f/rp3_a_c1_process_seal.json": "252e23d92d8e24ed894c1fbc24def06e7922040d475187918dc254c49e7fab9d",
...
def load_contract(root: Path) -> dict[str, Any]:
    ...
    required = {"work_order_id": rp3.WORK_ORDER, "base_sandbox_sha": rp3.BASE_SANDBOX_SHA, "resolution": 160, "worker_count": 6, "native_solves_per_worker": 9, "runner_relative_path": str(Path("audit/e9f/rp3_b_r160_runner.py"))}
    if any(value.get(key) != expected for key, expected in required.items()):
        raise RuntimeError("RP3_B_CONTRACT_FAIL_CLOSED")
    return value
...
def run_child(root: Path, row: Mapping[str, Any], runtime_root: Path, execution: str, contract_sha: str, policy_sha: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    c35.atomic_write(slot / "binding.json", {"schema": "mephc_e9f_c1_rp3_b_binding_v1", **expected, "payload_path": str(payload_path), "failure_path": str(failure_path)})
    command = [sys.executable, str(root / WORKER), "--root", str(root), "--worker-id", row["sample_id"], "--resolution", "160", "--payload-path", str(payload_path), "--failure-path", str(failure_path), "--execution-sha", execution, "--contract-sha256", contract_sha, "--rp1-policy-sha256", policy_sha]
    ...
    measurement = {"pid": child.pid, "return_code": child.returncode, "direct_pid_gone": not (Path("/proc") / str(child.pid)).exists(), "orphan_pids": orphans, "orphan_count": len(orphans), "stdout_sha256": hashlib.sha256(output).hexdigest(), "stderr_sha256": hashlib.sha256(error).hexdigest(), "stdout_tail": output[-65536:].decode(errors="replace"), "stderr_tail": error[-65536:].decode(errors="replace"), "elapsed_seconds": time.monotonic() - started, "worker_id": row["sample_id"], "resolution": 160, "payload_path": str(payload_path), "failure_path": str(failure_path), "failure_sidecar_exists": failure_path.exists()}
    if child.returncode != 0 or orphans or not measurement["direct_pid_gone"]:
        failure = {"schema": "mephc_e9f_c1_rp3_b_parent_failure_v1", "work_order_id": rp3.WORK_ORDER, "execution_sha": execution, "stage": "child_lifecycle", "process_measurement": measurement, "child_failure_sidecar": json.loads(failure_path.read_text()) if failure_path.exists() else None}
        c35.atomic_write(runtime_root / "parent_failure.json", failure)
        raise RuntimeError("RP3_B_CHILD_FAIL_CLOSED")
...
def execute(root: Path, runtime_root: Path) -> dict[str, Any]:
    ...
    if runtime_root.exists():
        ...
        if not checkpoint_path.is_file():
            raise RuntimeError("RP3_B_RESUME_CHECKPOINT_MISSING_FAIL_CLOSED")
    ...
    result = {"schema": "mephc_e9f_c1_rp3_b_r160_result_v1", "status": "E9F_C1_RP3_B_FIXED_SIX_R160_DIAGNOSTIC_COMPLETE_READY_FOR_SUPERVISOR_CONVERGENCE_DECISION", "project_id": "MEPHC", "work_order_id": rp3.WORK_ORDER, "base_sandbox_sha": rp3.BASE_SANDBOX_SHA, "execution_sha": execution, "contract_sha256": contract_sha, "provenance": provenance, "rows": rows, "payloads": payloads, "measurements": measurements, "checkpoint_generation": len(completed), "authorized_native_solve_count": 54, "actual_native_solve_count": sum(payload["solve_count"] for payload in payloads), "replay_policy": "NOT_APPLICABLE_R160_ORIGINAL_RP2_HAS_NO_R160_KEY", "convergence_rows": convergence, "spectral_rows": spectral, "no_convergence_verdict": True, "diagnostic_only": True, "reducer_admissible": False, "rp3_reducer_started": False, "rp3_chern_started": False}
    c35.atomic_write(runtime_root / "rp3_b_r160_result.json", result)
    manifest = {"schema": "mephc_e9f_c1_rp3_b_r160_manifest_v1", "work_order_id": rp3.WORK_ORDER, "execution_sha": execution, "checkpoint_sha256": c35.sha(runtime_root / "matrix_checkpoint.json"), "result_sha256": c35.sha(runtime_root / "rp3_b_r160_result.json"), "worker_count": 6, "matrix_entry_count": 12, "authorized_native_solves": 54, "actual_native_solves": result["actual_native_solve_count"]}
    c35.atomic_write(runtime_root / "rp3_b_r160_manifest.json", manifest)
    return {"status": result["status"], "execution_sha": execution, "worker_count": 6, "matrix_entry_count": 12, "authorized_native_solves": 54, "actual_native_solves": result["actual_native_solve_count"], "result_sha256": c35.sha(runtime_root / "rp3_b_r160_result.json"), "manifest_sha256": c35.sha(runtime_root / "rp3_b_r160_manifest.json")}
