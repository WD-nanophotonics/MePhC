"""Fixed, zero-argument R192 acquisition for the direct MePhC flow."""
...
GRAPH_PATH = Path(__file__).resolve().with_name("qp_b_c2_c3_r8_c3_r192_request_graph.json")
BINDING_PATH = Path(__file__).resolve().with_name("qp_b_c2_c3_r8_c3_r192_acquisition_binding.json")
...
RESOLUTION = "R192"
RESOLUTION_VALUE = 192
SAMPLES = (
    (-10, -3, "CALIBRATION_CONTROL"),
    (-34, 9, "CALIBRATION_CONTROL"),
    (-6, -1, "STENCIL_DIAGNOSTIC"),
    (-34, -16, "POLICY_CHALLENGE"),
    (-34, -17, "POLICY_CHALLENGE"),
    (-34, 17, "POLICY_CHALLENGE"),
...
MAX_UNIQUE_REQUESTS = 70
MAX_FRESH_SOLVER_EXECUTIONS = 70
...
def load_runtime():
    return load_module("_mephc_r192_science_runtime", SCIENCE_RUNTIME_PATH)


def load_scientific_job():
    return load_module("_mephc_r192_scientific_job", SCIENTIFIC_JOB_PATH)
...
def make_graph() -> dict[str, Any]:
    ...
    return {
        ...
        "logical_provider_demand_count_per_resolution": 72, "unique_request_count_by_resolution": {RESOLUTION: 70},
        "global_unique_provider_request_count": 70, "duplicate_logical_demand_count": 2,
...
def verify_graph(graph: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(demands, list) or len(demands) != 72:
    ...
    if graph.get("unique_request_count_by_resolution") != {RESOLUTION: 70}:
        raise EntrypointError("GRAPH_PER_RESOLUTION_COUNT_INVALID")
    if graph.get("duplicate_logical_demand_count") != 2:
    ...
    if samples != SAMPLE_SET or len(groups) != 8 or any(points != POINTS for points in groups.values()):
    ...
    if len(collisions) != 2 or sum(len(refs) - 1 for refs in collisions.values()) != 2:
        raise EntrypointError("GRAPH_DERIVED_DUPLICATE_COUNT_INVALID")
    expected = {
        tuple(item for ref in sorted((
            (f"fr=0;grid_i=-34;grid_j=-17;role=POLICY_CHALLENGE;resolution={RESOLUTION}", "H72_PLUS_Y"),
            (f"fr=0;grid_i=-34;grid_j=-16;role=POLICY_CHALLENGE;resolution={RESOLUTION}", "H72_MINUS_Y"),
        )) for item in ref),
        tuple(item for ref in sorted((
    ...
    return {"logical_provider_demand_count": 72, "unique_provider_request_count": 70,
            "duplicate_logical_demand_count": 2, "unique_request_count_by_resolution": {RESOLUTION: 70},
...
def _acquisition_manifest(store: Any, plan: list[dict[str, Any]], runtime_sha: str, entrypoint_sha: str, graph_sha: str,
    ...
    content = {
        ...
        "resolution": RESOLUTION, "logical_provider_demand_count": 72, "unique_provider_request_count": 70,
        "completed_key_count": 70, "records": records, "fresh_provider_execution_count": fresh,
...
def _write_binding(dataset: dict[str, Any], runtime_sha: str, entrypoint_sha: str, graph_sha: str,
    ...
    value = {
        ...
        "logical_provider_demand_count": 72, "unique_provider_request_count": 70,
        "duplicate_logical_demand_count": 2, "completed_key_count": 70, "failed_key_count": 0,
...
def acquire() -> dict[str, Any]:
    ...
    for item in plan:
        ...
        try:
            ...
            runtime.decode_snapshot(payload)
            store.put(key, payload, {"schema": "mephc_r192_exact_key_record_v1", "key_sha256": hashlib.sha256(key).hexdigest(), **_identity(item["request_key"])})
        except Exception as exc:
            raise EntrypointError("R192_REQUEST_FAILED", f"{hashlib.sha256(key).hexdigest()}:{type(exc).__name__}") from exc
        fresh += 1
        mpb = True
    ...
    return {
        ...
        "terminal": "E9F_C2_QP_B_C2_C3_R8_C3_A1_R192_DATASET_ACQUIRED_READY_FOR_SOLVER_FREE_ANALYSIS",
    }
...
def main() -> int:
    try:
        ...
        print("MEPHC_NATIVE_RESULT_JSON=" + canonical({"schema": "mephc-r8-c3-r192-acquisition-v1", "state": "failed", "error_code": exc.code, "detail": exc.detail[:1000], "terminal": "E9F_C2_QP_B_C2_C3_R8_C3_A1_FAIL_CLOSED"}).decode("utf-8"))
        return 2
